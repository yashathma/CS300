...
//Creating static variables used in class
  ...
  public static void mousePressed() {
    ...
    for(int i = 0;i<Walker.NUM_FRAMES;i++) {
      if(walkers[i]!=null) {
        if (Utility.mouseX()>(walkers[i].getPositionX()-(frames[0].width/2)) &&
            Utility.mouseX()<(walkers[i].getPositionX()+(frames[0].width/2))) {
          if (Utility.mouseY()>(walkers[i].getPositionY()-(frames[0].height/2)) &&
              Utility.mouseY()<(walkers[i].getPositionY()+(frames[0].height/2))) {
